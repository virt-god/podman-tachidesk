...
echo "Suwayomi data location inside the container: /data"
...
envsubst < /app/server.conf.template > /data/server.conf

exec java -Dsuwayomi.tachidesk.config.server.rootDir="/data" -jar "/app/tachidesk_latest.jar";
